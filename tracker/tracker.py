...
import numpy as np
from tracker.common import dprint
from scipy.optimize import linear_sum_assignment
from tracker.ukf import UKF
        ...
        if flag == 1:
            ...
            for i in range(len(assignment)):
                ...
                if self.tracks[i].prediction.shape[0] == 2:
                    self.tracks[i].prediction = np.insert(self.tracks[i].prediction, self.tracks[i].prediction.shape[0],
                                                          0)
                    self.tracks[i].prediction = np.insert(self.tracks[i].prediction, self.tracks[i].prediction.shape[0],
                                                          0)
                    self.tracks[i].prediction = np.insert(self.tracks[i].prediction, self.tracks[i].prediction.shape[0],
